...
def floorplan_new_place_stands(fig, ax, image_multiplier, fl_div):
        for x in stand_location.objects.filter(sl_stand__s_rx_event__re_name__iexact='ISC West 2025'):
                ...
                sa_analysis_number, sa_analysis_title, spg = stand_get_analysis_record(x.sl_stand, None, 'Sq Foot Gradient')
                if(spg is None):
                        spg = 0
                ...
                if(x.sl_x_length * x.sl_y_length > 1):
                        ...
                        if(x.sl_stand.s_stand_status in ('Sold', 'New Sell')):
                                ...
                                for r in rsa:
                                        sa_analysis_number, sa_analysis_title, sa_analysis_value = stand_get_analysis_record(r.sa_stand, None, r.sa_analysis_title)
...
#        print_ax_size(fig, gs, ax, "create_floorplan_subplot")
        ...
        return(fig)
def render_floorplan(rxe, header_set, footer_set, message_set, analysis_set_top, analysis_set_bottom, image_multiplier, floorplan_type):
